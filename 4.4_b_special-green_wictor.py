...
def initialize_asymmetric_boundaries(n):
    ...
    - Top and bottom sides: 10V
    - Left and right sides: 5V
    - Five specific boundary sites held at 20V
    """
    grid = np.zeros((n + 2, n + 2))  # Include boundary points
    grid[0, :] = 10  # Top boundary
    grid[-1, :] = 10  # Bottom boundary
    ...
    # Set specific boundary sites to 20V
    special_boundary_sites = [(5, 0), (4, 0), (0, 3), (6, 0), (10, 3)]
    for x, y in special_boundary_sites:
        grid[x, y] = 20
    return grid, special_boundary_sites
...
if __name__ == "__main__":
    ...
    # Initialize grid
    grid, special_boundary_sites = initialize_asymmetric_boundaries(n)
    ...
    for i in range(1, n + 1):
        for j in range(1, n + 1):
            ...
            # Add contributions from special boundary sites
            for x, y in special_boundary_sites:
                potential[i, j] += G[x, y] * 20  # Add contribution of 20V sites

    # Plot the potential with boundaries
    title = f'Potential, 5 sites at V=20, grid size = {n}, n-walks = {num_walks}'
