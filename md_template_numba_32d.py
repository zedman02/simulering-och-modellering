...
def calculate_energy_and_heat_capacity(T_min, T_max, T_step, dt, nsteps, equilibration_steps):
    """
    Calculate the average energy and heat capacity for a range of temperatures.
    """
    ...
    for T in temperatures:
        ...
        MD = MDsimulator(T=T, dt=dt, nsteps=nsteps)
        MD.simulate()

        # Calculate averages post-equilibration
        total_steps = MD.step
        avg_Etot = MD.sumEtot / (total_steps - equilibration_steps)
        avg_Etot2 = MD.sumEtot2 / (total_steps - equilibration_steps)
        ...
        results.append({
            "Temperature": T,
            "Average Energy": avg_Etot,
            "Heat Capacity": Cv
        })

        print(f"T={T}: Avg Energy={avg_Etot:.4f}, Heat Capacity={Cv:.4f}")

    return results

def plot_results(results):
    """
    Plot average energy and heat capacity as a function of temperature.
    """
    temperatures = [r["Temperature"] for r in results]
    avg_energies = [r["Average Energy"] for r in results]
    heat_capacities = [r["Heat Capacity"] for r in results]

    # Plot average energy
    plt.figure()
    plt.plot(temperatures, avg_energies, marker='o')
...
if __name__ == "__main__":
    ...
    dt = 0.01
    nsteps = 50000
    equilibration_steps = 1000  # Steps to skip for equilibration

    results = calculate_energy_and_heat_capacity(T_min, T_max, T_step, dt, nsteps, equilibration_steps)
